...
import dorun.project.routineapp.user.controller.dto.AuthToken;
...
public class AuthController {
    ...
    @PostMapping("/refresh")
    public ApiResponse<AuthToken> refreshToken(HttpServletRequest request, HttpServletResponse response) {
        ...
        /**
         * 우근님 코드
         * 만약, 토큰이 만료가 안 되었을 경우 토큰 갱신에 대한 요청이 들어왔을 때, 사용자에게 500 에러를 보여주는 것이 맞는가?
         * 그냥 토큰을 갱신을 해주는 것이 맞지 않나?
         * ==> 이야기 해보고 싶습니다.
        ...
        if (authRefreshToken.validate()) {
            return new ApiResponse<>(ApiResponseMessage.INVALID_TOKEN);
        }
        ...
        AuthToken newAuthToken = new AuthToken(newAccessToken.getToken(), userRefreshToken.getRefreshToken());

        return new ApiResponse(ApiResponseMessage.TOKEN_REFRESH_SUCCESS, newAuthToken);
    }
}
